"""Provides :class:`PointProcess` iterator and :class:`IntensityFunction` abstract classes
and implementation of :class:`MarkovChainMonteCarlo` and :class:`SpatialPoisonPointProcess`
iterators
"""
...
"""Generic type of :class:`PointProcess`, bounded by tuple[int, ...]."""
...
class IntensityFunction:
    """Abstract class that provides following interface: :meth:`rate`, :meth:`is_accepted`
    for :meth:`PointProcess.__next__` method.
    """

    @property
    def rate(self) -> float:
        """Expected number of points generated.
...
"""Type of :attr:`rate` attribute of :class:`PointProcess`."""


class PointProcess(Iterator[T]):
    """Abstract class for implementing point processes.

    It is a sub-class of :class:`Iterator` [:const:`T`] that represents a stream of :const:`T` type
    ...
    def __init__(self, rate: RateType, size: T, seed: int) -> None:
        ...
        A private :attr:`_next` generator is set that provides the stream of points. In order to
        implement simulation of a point process :meth:`_generate` used to set :attr:`_next` should
        be overridden.
    ...
    def __next__(self) -> T:
        """Override of :meth:`object.__next__` method.

        :return: Points from private generator.
        :rtype: :const:`T`
        """
...
class MarkovChainMonteCarlo(PointProcess):
    """Implementation of :class:`PointProcess` that uses Markov chain Monte Carlo method."""
...
class SpatialPoissonPointProcess(PointProcess):
    """Implementation of :class:`PointProcess`."""
